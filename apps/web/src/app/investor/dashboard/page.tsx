...
import {
  ...
  Star, Eye, MapPin, Check, ArrowUpRight
} from 'lucide-react';
...
// Dummy data removed. Will fetch from API.
function StartupCard({ startup }: { startup: any }) {
  ...
  return (
    ...
    >
      ...
      <p className="text-gray-400 text-sm mb-4">{startup.tagline}</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
              {startups.map((startup) => (
                <StartupCard key={startup.id || startup.name} startup={{
                  ...startup,
                  avatar: startup.name.substring(0, 2).toUpperCase(),
                  color: '#8b5cf6', // random color logic could go here
                }} />
              ))}
            </div>
          </div>
